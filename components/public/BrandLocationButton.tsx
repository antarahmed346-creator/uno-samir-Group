...
export default function BrandLocationButton({
  ...
  if (locations.length === 1) {
    return (
      <a
        ...
        className="inline-flex items-center gap-2 mt-8 mr-3 px-8 py-4 bg-white/10 backdrop-blur-sm border border-white/30 text-white rounded-full font-bold hover:bg-white/20 transition"
      >
  ...
  return (
    <div ref={ref} className="relative inline-block mt-8 mr-3">
      <button
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-2 px-8 py-4 bg-white/10 backdrop-blur-sm border border-white/30 text-white rounded-full font-bold hover:bg-white/20 transition"
      >
        <MapPin className="w-5 h-5" />
